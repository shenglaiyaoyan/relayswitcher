...
test('官方模式:补入 gpt-6-sol/luna(官方参数),隐藏被替代的 5.6 系与 terra,其余不动', () => {
  ...
  assert.equal(bySlug['gpt-6-sol'].context_window, 1050000, '官方公布上下文 1.05M');
  assert.equal(bySlug['gpt-6-sol'].max_context_window, 1050000);
...
test('中转模式:提供了 gpt-6-sol 时隐藏其 5.6 前任并校准参数', () => {
  ...
  assert.equal(bySlug['gpt-6-sol'].context_window, 1050000, '校准为官方参数');
...
test('内嵌快照追上后(原生条目已存在):该条目退化为参数校准,仍缺失的照常补入', () => {
  ...
  assert.equal(sol.context_window, 1050000, '按官方参数校准');
